...
public:
    ...
    {
        _combatants->erase(std::find(_combatants->begin(), _combatants->end(), ptr));
    }
    ...
    void addEffectOnGroup(SimpleEffectBuilder* builder, std::vector<t_pos_comb>& collection);
    ...
    void setEffectDurationOnGroup(int duration, std::vector<t_pair_comb_with_effect>& collection)
    {
        for (auto it : collection)
    ...
    const std::vector<CommandBase*>& getCommands() const { return _mediator->getCommands(); }
