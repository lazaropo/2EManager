 ...
 public:
  void execute() override {
    ...
    for (auto it : _commands_and_effects) {
      if (it.find("effect:") == 0)
        _mediator->makeEffect(this, getReciever(),
                              it /*num of sym in "effect:"*/);
      else if (it.find("command:") == 0)
        _mediator->makeCommand(this, getReciever(),
  ...
  void undo() override {
    ...
    for (auto it : _commands_and_effects) {
      if (it.find("effect:") == 0)
        _mediator->undoEffect(this, getReciever(),
                              it /*8 - num of sym in "effect:"*/);
      else if (it.find("command:") == 0)
        _mediator->undoCommand(this, getReciever(),
